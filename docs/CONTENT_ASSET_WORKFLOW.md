...
当前 Join 区块保留静态信息字段，但 CTA 已降级为 `mailto:account@starfc.sg`，避免访客误以为站内表单可以直接提交。

后续如果有真实 Google Form / Tally 链接，再把 CTA 改成外链，并同步更新 `index.html` 和 `links-data`。

表单仍是比封面更高优先级的问题，因为它直接影响新球员联系球队。
