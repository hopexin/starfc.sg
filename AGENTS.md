...
| `data/blog.js` | 博客索引（首页最新文章区 + blog/index.html 都读它） | ✅ 发文时 |
...
博客 = **独立静态 HTML 页**（利于 SEO）+ `data/blog.js` 索引（驱动首页最新文章区与 blog/index.html）。
...
8. 本地预览：首页最新文章区出现新卡片、文章页排版正常、`blog/index.html` 列表正常 → 提交。
...
- 名人堂当前暂时从首页隐藏；等首位元老资料确认后再恢复区块与入口（字段清单见 `docs/CONTENT_AUDIT.md`）。
- 语言偏好存 localStorage（`starfc-language`)；文章页目前只有中文，属预期。
- 首页最新文章区当前只有 1 篇站内文章，属预期（小红书旧文待逐篇迁移，见任务 C）。
